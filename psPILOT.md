1. # psPILOT
   ...
   Jump to the line of the program indicated by `«label»`. If there is no line in the program matching the `«label»`, psPILOT will report that the label is undefined and exit the PILOT program.

   `J:*RESTART` will cause execution to continue from the line following the label `*restart`.

   M: Match

   M:«string-expression»Match the accept buffer against string variables or string literals. Example:
